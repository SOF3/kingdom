...
class ElementBody(internal val dom: DomElementImpl) : ComplexBlock by ComplexBlockImpl(dom), AttrGroupAttrs by AttrGroupAttrsImpl(dom), DomElement {
	init {
		dom.element = this
	}

	override val elementName = "body"

	fun write(writer: KingdomWriter) = dom.write(writer)

	inline operator fun invoke(id: XsID? = null, classes: List<String>? = null, style: SimpleStyleSheet? = null, title: SimpleText? = null, lang: SimpleLanguageCode? = null, dir: AttrTypeI18nDir? = null, onclick: SimpleScript? = null, ondblclick: SimpleScript? = null, onmousedown: SimpleScript? = null, onmouseup: SimpleScript? = null, onmouseover: SimpleScript? = null, onmousemove: SimpleScript? = null, onmouseout: SimpleScript? = null, onkeypress: SimpleScript? = null, onkeydown: SimpleScript? = null, onkeyup: SimpleScript? = null, onload: SimpleScript? = null, onunload: SimpleScript? = null, fn: ElementBody.() -> Unit = {}): ElementBody {
		...
		if (onunload != null) this.onunload = onunload
		fn(this)
		return this
	}

	inline operator fun invoke(fn: ElementBody.() -> Unit) = apply(fn)

	var onload: SimpleScript? by DomAttributeAdapter("onload", dom, null as SimpleScript?)
